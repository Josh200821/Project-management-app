...
class CommentService:
    ...
    async def create(self, task_id: UUID, data: CommentCreate, author_id: UUID):
        comment = await self.repo.create(task_id=task_id, body=data.body, author_id=author_id)
        ...
        if mentions:
            ...
            send_mention_notifications.delay(str(task_id), str(author_id), mentions)
        return comment
    ...
    async def update(self, comment_id: UUID, data: CommentUpdate):
        ...
        return await self.repo.update(comment, body=data.body, edited_at=datetime.now(UTC))
