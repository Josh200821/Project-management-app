...
def upgrade() -> None:
    ...
    # RLS
    # NOTE: the original version of this loop applied the same
    # `organization_id = current_setting(...)::uuid` policy expression to
    # every table in the list, but only `projects` actually has an
    # `organization_id` column -- `organizations` has no such column (it IS
    # the tenant), and `tasks`/`sprints` only reach it via `projects`. As
    # written, this would fail at migration time with
    # `column "organization_id" does not exist` the moment it reached
    # "organizations". Each table now gets a policy expression that matches
    # its actual schema.
