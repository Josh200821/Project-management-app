    ...
    async def test_register_creates_user(self, auth_service, mock_session):
        with patch("app.services.auth_service.UserRepository") as MockRepo:
            ...
            mock_repo.create = AsyncMock(return_value=MagicMock(
                id="test-uuid", email="test@example.com"
            ))
            with patch("app.services.auth_service.create_access_token", return_value="token"):
                result = await auth_service.register(
                    RegisterRequest(email="test@example.com", password="Secure123!", full_name="Test User")
                )
        ...
        with patch("app.services.auth_service.UserRepository") as MockRepo:
            ...
            with pytest.raises(ValueError, match="already registered"):
                await auth_service.register(
                    RegisterRequest(email="exists@example.com", password="Secure123!", full_name="Test")
                )
