    ...
    async def test_create_generates_slug(self, project_service):
        ...
        with patch("app.services.project_service.ProjectRepository") as MockRepo:
            ...
            mock_repo.create = AsyncMock(return_value=MagicMock(
                id=uuid4(), name="My Project", slug="my-project",
                organization_id=uuid4(), status="active", task_counter=0,
                description=None, created_at=MagicMock()
            ))
            project = await project_service.create(
                uuid4(), ProjectCreate(name="My Project"), owner_id
            )
