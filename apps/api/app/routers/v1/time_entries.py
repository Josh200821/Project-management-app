...
async def create_time_entry(body: TimeEntryCreate, current_user: CurrentUser, session: DbSession):
    ...
    entry = TimeEntry(
        task_id=UUID(body.task_id), user_id=current_user.id,
        started_at=body.started_at, stopped_at=body.stopped_at,
        duration_seconds=duration, description=body.description, billable=body.billable,
    )
    session.add(entry)
    await session.flush()
...
async def list_for_task(task_id: str, current_user: CurrentUser, session: DbSession):
    ...
    return ApiResponse(data=[{"id": str(e.id), "started_at": str(e.started_at), "duration_seconds": e.duration_seconds} for e in entries])
