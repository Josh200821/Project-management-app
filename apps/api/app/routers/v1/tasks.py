...
async def create_task(project_id: str, body: TaskCreate, current_user: CurrentUser, session: DbSession):
...
async def list_tasks(project_id: str, current_user: CurrentUser, session: DbSession,
                     status: str = None, page: int = 1, per_page: int = 50):
    from uuid import UUID
    svc = TaskService(session)
...
async def update_task(project_id: str, task_id: str, body: TaskUpdate, current_user: CurrentUser, session: DbSession):
    from uuid import UUID
    svc = TaskService(session)
...
async def bulk_update(project_id: str, body: TaskBulkUpdate, current_user: CurrentUser, session: DbSession):
    from uuid import UUID
    svc = TaskService(session)
