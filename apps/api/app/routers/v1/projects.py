...
async def create_project(org_id: str, body: ProjectCreate, current_user: CurrentUser, session: DbSession):
...
async def list_projects(org_id: str, current_user: CurrentUser, session: DbSession, page: int = 1, per_page: int = 20):
    from uuid import UUID
    svc = ProjectService(session)
...
async def update_project(org_id: str, project_id: str, body: ProjectUpdate, current_user: CurrentUser, session: DbSession):
    from uuid import UUID
    svc = ProjectService(session)
...
async def delete_project(org_id: str, project_id: str, current_user: CurrentUser, session: DbSession):
    from uuid import UUID
    svc = ProjectService(session)
